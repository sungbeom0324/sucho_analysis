                 ...
                 const char* goldenJsonFile = "")
{
    ...
    tout->Branch("Z_mass_ee",  &Z_mass_ee,  "Z_mass_ee/F");
    tout->Branch("Z_mass_eeg", &Z_mass_eeg, "Z_mass_eeg/F");
    tout->Branch("AddPhoton_pt", &AddPhoton_pt, "AddPhoton_pt/F");
    tout->Branch("AddPhoton_eta", &AddPhoton_eta, "AddPhoton_eta/F");
    tout->Branch("dR_eg", &dR_eg, "dR_eg/F");
    ...
    for (Long64_t i = 0; i < nAll; ++i) {
        ...
        for (int im = 0; im < nMuonToCheck; ++im) {
            ...
            // 1) WP80 cut
            // if (Muon_isTracker[im] != 1) continue;
        ...
        for (int ig = 0; ig < nPhoToCheck; ++ig) {
            ...
            // 5) Photon_electronIdx == -1 (Electron_photonIdx 와 대응되는 브랜치인 것 같음. AN-22-024_v4_line424)
            // if (Photon_electronIdx[ig] != -1) continue; THIS IS WRONG..!
        ...
        for (size_t a = 0; a < goodEle.size(); ++a) {
            for (size_t b = a + 1; b < goodEle.size(); ++b) {
                unsigned int i1 = goodEle[a];
                unsigned int i2 = goodEle[b];

                if (Electron_charge[i1] * Electron_charge[i2] != -1) continue; // OS 조건

                TLorentzVector tmp_e1, tmp_e2;
                tmp_e1.SetPtEtaPhiM(Electron_pt[i1], Electron_eta[i1],
                                Electron_phi[i1], Electron_mass[i1]);
                tmp_e2.SetPtEtaPhiM(Electron_pt[i2], Electron_eta[i2],
                                Electron_phi[i2], Electron_mass[i2]);
                TLorentzVector tmp_z = tmp_e1 + tmp_e2;
                double mass = tmp_z.M();
        ...
        // S5. (Part of) // Case 2. OS pair 자체가 없는 경우. 없는 경우 z_ee, z_eeg = -1.
        if (best_i < 0 || best_j < 0) {
            //std::cout << "Case 2" << endl;
            //std::cout << Z_mass_ee << endl << endl;
            tout->Fill();
            continue;
        }
        ...
        TLorentzVector e1, e2, z_ee;
        e1.SetPtEtaPhiM(Electron_pt[best_i], Electron_eta[best_i],
                        Electron_phi[best_i], Electron_mass[best_i]);
        e2.SetPtEtaPhiM(Electron_pt[best_j], Electron_eta[best_j],
                        Electron_phi[best_j], Electron_mass[best_j]);
        z_ee = e1 + e2;
        Z_mass_ee  = z_ee.M();
        // std::cout << "Z_ee measured : " << Z_mass_ee << endl;
        Z_mass_eeg = Z_mass_ee;  // 기본값: photon 없으면 이 값 유지

        // S5. (Complete) Onshell Z.
        if (Z_mass_ee < 80 || Z_mass_ee > 100) { // Reduced Baseline Selection
            continue;
        }
        ...
        if (goodPhoton.size() >= 1) {
            ...
            int best_pho = -1;
            float best_pt = -999.0; // Metric is dR(Fsr, mu)/EE. AN-22-027_v4.pdf 876 line.
            ...
            if (best_pho >= 0) {
                ...
                TLorentzVector z_eeg = z_ee + g;
                Z_mass_eeg = z_eeg.M();
                // std::cout << "Z_eeg updated : " << Z_mass_eeg << std::endl;

                AddPhoton_pt = Photon_pt[best_pho]; // To save.
                AddPhoton_eta = Photon_eta[best_pho]; // To save.
                double dr1 = g.DeltaR(e1);
                double dr2 = g.DeltaR(e2);
                dR_eg = std::min(dr1, dr2);
            }
        }

        // S6. g_pt / m_llg > 15./110
        if ((AddPhoton_pt/Z_mass_eeg) <= 15./110) continue;

        // S7. m_llg + m_ll > 185
        if (Z_mass_eeg + Z_mass_ee <= 185) continue;        

        // S8. 95 < mllg < 180
        if (Z_mass_eeg <= 95 || Z_mass_eeg>= 180) continue;
        ...
        // S10. Control Region
        if (Z_mass_eeg > 120 && Z_mass_eeg < 130) { // Control Region
            continue;
        }
